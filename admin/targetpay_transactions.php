...
function targetpay_get_directorylist()
{
    ...
    $objDigiCore = new TargetPayCore("AUTO");
...
function targetpay_check_transaction_status($transactionID = '')
{
    ...
    $objDigiCore = new TargetPayCore("AUTO", $selected_transaction["rtlo"]);
    $objDigiCore->setBankId($selected_transaction["issuer_id"]);
    
    @$objDigiCore->checkPayment($transactionID);
    
    $realstatus = "Open";
    if ($objDigiCore->getPaidStatus()) {
        $realstatus = "Success";
    } else {
        list ($errorcode, $void) = explode(" ", $objDigiCore->getErrorMessage());
        switch ($errorcode) {
            case "TP0010":
                $realstatus = "Open";
                break;
    ...
    if ($realstatus != "") {
        ...
        if ($status_update->resource) {
            ...
            $messageStack->add_session(TARGETPAY_MESSAGE_WARNING_STATUS . " statuscode:" . $objDigiCore->getErrorMessage(), 'warning');
        }
    } else {
...
function getSuggestedAction($transactionID, $transactionStatus, $orderID, $orderStatus, $session_id = '', $age = 0)
{
    if (strtolower($transactionStatus) == "open") {
        return array(
            "text" => TARGETPAY_TEXT_CHECK_STATUS,
    ...
    if (($transactionStatus == "success") && ! ($orderID > 0) && ($age > 0) && ($age < (int) MODULE_PAYMENT_TARGETPAY_EXPIRATION_PERIOD)) {
        return array(
            "text" => TARGETPAY_WAIT_OR_CREATE_ORDER,
    ...
    if (($transactionStatus == "success") && ! ($orderID > 0)) {
        return array(
            "text" => TARGETPAY_TEXT_CREATE_ORDER,
            "link" => HTTP_CATALOG_SERVER . DIR_WS_CATALOG . 'index.php?main_page=checkout_process&targetpay_repair_order=true&targetpay_transaction_id=' . $transactionID,
...
/**
 * cleanup non open or success transactions
 */
                                            ...
                                            <tr>
                                                <td class="smallText" align="center">
                                                    <?php echo tep_draw_form('transactions', FILENAME_TARGETPAY_TRANSACTIONS, '', 'get'); ?>                                                        
                                                        ...
                                                        echo tep_draw_radio_field('transFilter', TARGETPAY_TEXT_FILTER_ALL, false, 'onchange="this.form.submit();"') . TARGETPAY_TEXT_FILTER_ALL . tep_draw_radio_field('transFilter', TARGETPAY_TEXT_FILTER_INCOMPLETE, false, 'onchange="this.form.submit();"') . TARGETPAY_TEXT_FILTER_INCOMPLETE . tep_draw_radio_field('transFilter', TARGETPAY_TEXT_FILTER_COMPLETE, false, 'onchange="this.form.submit();"') . TARGETPAY_TEXT_FILTER_COMPLETE;
                        ?> 
                                                        <input
                                            ...
                                            <tr>
                                                <?php echo tep_draw_form('transactions', FILENAME_TARGETPAY_TRANSACTIONS, '', 'get'); ?>
                                                    <td class="smallText" align="center"><?php echo tep_draw_hidden_field('action', 'cleanup'); ?>
                                                        <input
                                                    onClick="return confirm('Weet u zeker dat u de transatie tabel nu wilt opschonen?')"
                                                    type="submit"
                                                    value="<?php echo TARGETPAY_TEXT_FILTER_CLEANUP; ?>" />
                                    ...
                                    <td align="right">
                                        <table border="0" width="100%" cellspacing="0" cellpadding="0">
                                            <tr>
                                                <?php echo tep_draw_form('transactions', FILENAME_TARGETPAY_TRANSACTIONS, '', 'get'); ?>
                                                    <td
                                                    class="smallText" align="right">
                                                    <?php
                        
                                                    echo TARGETPAY_TEXT_SEARCH_TRANSACTION_ID . ' ' . tep_draw_input_field('transactionID', '', 'size="12"') . tep_draw_hidden_field('action', 'lookup') . tep_draw_hidden_field('transFilter', $transFilter);
                        ?>
                                                        </td>
                                                </form>
                                            </tr>
                                            <tr>
                                                <?php echo tep_draw_form('transactions', FILENAME_TARGETPAY_TRANSACTIONS, '', 'get'); ?>
                                                    ...
                                                    echo TARGETPAY_TEXT_SEARCH_ORDER_ID . ' ' . tep_draw_input_field('orderID', '', 'size="12"') . tep_draw_hidden_field('action', 'lookup') . tep_draw_hidden_field('transFilter', $transFilter);
                        ?>
                                                    </td>
                                                </form>
                                            </tr>
                                        ...
                                        <table border="0" width="100%" cellspacing="0" cellpadding="2">
                                            ...
                                            while ($transaction = tep_db_fetch_array($transactions)) {
                                                ...
                                                <td class="dataTableContent"><a
                                                    href="<?php echo tep_href_link(FILENAME_ORDERS)."?oID=".$transaction['order_id']; ?>&action=edit"><?php echo $transaction['order_id']; ?></a></td>
                                                ...
                                                <td class="dataTableContent" align="right"><a
                                                    target="_blank"
                                                    href="<?php echo tep_href_link(FILENAME_CUSTOMERS, 'selected_box=customers&cID=' . $transaction['customer_id'], 'NONSSL')?>"><?php echo $transaction['customer_id'];?></a>
                                                </td>
                                                <td class="dataTableContent" align="right">
                                                    <?php
                                                    ...
                                                    if (($res['link'] != '#') && ! empty($res['link'])) {
                                                        ?> <a <?php echo $res['parameters']; ?>
													href="<?php echo $res['link']?>"><?php echo $res['text']; ?></a>
													<?php
                                                    } else {
                                                        echo '--';
                                                    }
                                                ?></td>
                                                <td class="dataTableContent" align="right"><a
                                                    href="<?php echo tep_href_link(FILENAME_TARGETPAY_TRANSACTIONS, tep_get_all_get_params(array('action','transactionID')) . 'action=lookup&transactionID=' .$transaction['transaction_id'], 'NONSSL')?>"><?php echo TARGETPAY_TEXT_DETAILS;?></a>&nbsp;</td>
                                            </tr>
                                            <?php
                                            }
        ?>
                                                ...
                                                <td valign="top">
                                                    <table border="0" width="100%" cellspacing="0"
                                                        ...
                                                        <tr class="dataTableRow" onmouseover="rowOverEffect(this)"
                                                            ...
                                                            <td class="dataTableContent"><a
                                                                onclick="return confirm('Weet u zeker dat u deze transactie wilt verwijderen uit de Digiwallet iDEAL tabel?')"
                                                                href="<?php echo tep_href_link(FILENAME_TARGETPAY_TRANSACTIONS, tep_get_all_get_params(array('action','transactionID')) . 'action=remove&transactionID=' .$selected_transaction['transaction_id'], 'NONSSL')?>">Verwijder</a>
                                                            </td>
                                                        </tr>
                                                        <tr class="dataTableRow" onmouseover="rowOverEffect(this)"
                                                            onmouseout="rowOutEffect(this)">
                                                        ...
                                                        <tr class="dataTableRow" onmouseover="rowOverEffect(this)"
                                                            ...
                                                            <td class="dataTableContent"><a
                                                                href="<?php echo tep_href_link(FILENAME_ORDERS)."?action=edit&page=1&oID=".$selected_transaction['order_id']; ?>"><?php echo $selected_transaction['order_id']; ?></a></td>
                                                        </tr>
                                                        <tr class="dataTableRow" onmouseover="rowOverEffect(this)"
                                                            onmouseout="rowOutEffect(this)">
                                                            <td class="dataTableContent"><?php echo TARGETPAY_TABLE_HEADING_TRANSACTION_STATUS; ?></td>
                                                            <td class="dataTableContent"><?php echo $selected_transaction['transaction_status']; ?><a
                                                                href="<?php echo tep_href_link(FILENAME_TARGETPAY_TRANSACTIONS, tep_get_all_get_params(array('action')) . 'action=checkstatus&transactionID=' . $selected_transaction['transaction_id']); ?>">.</a></td>
                                                        </tr>
                                                        <tr class="dataTableRow" onmouseover="rowOverEffect(this)"
                                                            onmouseout="rowOutEffect(this)">
                                                        ...
                                                        <tr class="dataTableRow" onmouseover="rowOverEffect(this)"
                                                            ...
                                                            <td class="dataTableContent"><a target="_blank"
                                                                href="<?php echo tep_href_link(FILENAME_CUSTOMERS, "selected_box=customers&cID=" . $selected_transaction['customer_id'], 'NONSSL')?>"><?php echo $selected_transaction['customer_id']; ?></a></td>
                                                        </tr>
                                                        <tr class="dataTableRow" onmouseover="rowOverEffect(this)"
                                                            onmouseout="rowOutEffect(this)">
                                                        ...
                                                        <tr class="dataTableRow" onmouseover="rowOverEffect(this)"
                                                            ...
                                                            <td class="dataTableContent"><a
                                                                <?php echo $res['parameters']; ?>
                                                                href="<?php echo $res['link']?>"><?php echo $res['text']; ?></a></td>
                                                        </tr>
                                                        <!-- 
                                                        <tr class="dataTableRow" onmouseover="rowOverEffect(this)" onmouseout="rowOutEffect(this)">
